...
I'm using the ~s-2~ keybinding because I already set ~s-`~ for other
window and ~s-1~ for other-frame. I might as well use something close
to the window/frame navigation keys.

#+begin_src emacs-lisp
  (use-package ace-window
    :ensure t
    :config (bind-key "s-2" 'ace-window))
#+end_src
